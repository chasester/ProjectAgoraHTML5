...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_Agora', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202608252110, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = sessionStorageCommandLine != null ? [sessionStorageCommandLine] : ['../../../Agora/Agora.uproject','-stdout',];
...
function detectWebGL() {
	...
	try {
		for(var failIfMajorPerformanceCaveat = 1; failIfMajorPerformanceCaveat >= 0; --failIfMajorPerformanceCaveat) {
			for(var i in names) {
				try {
					var context = canvas.getContext(names[i], {antialias:false,alpha:true,depth:true,stencil:false,failIfMajorPerformanceCaveat:!!failIfMajorPerformanceCaveat});
...
function reportDataBytesStoredInIndexedDB(deltaBytes) {
	...
	document.getElementById('clear_indexeddb').title = 'Clear Cache (' + formatBytes(Module['dataBytesStoredInIndexedDB']) + ')';
}
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('Agora-HTML5-Shipping.wasm'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('Agora-HTML5-Shipping.js'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('Agora-HTML5-Shipping'+dataVariant+'.data.js'));
...
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('Agora-HTML5-Shipping.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('Agora-HTML5-Shipping.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('Agora-HTML5-Shipping'+dataVariant+'.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('Agora-HTML5-Shipping'+dataVariant+'.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
		...
		Promise.all([mainCompiledCode, mainJsDownload, utilityJsDownload, dataDownload, compiledCodeInstantiateAction, compileShaders]).then(function() {
			if (!precompileShaders) {
				Module['precompiledShaders'] = Module['precompiledPrograms'] = Module['glIDCounter'] = Module['precompiledUniforms'] = null;
			}
